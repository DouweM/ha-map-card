...
| `sync_group`           |                                                                                                                              | Group name. Cards sharing the same `sync_group` sync their pan/zoom (e.g. across dashboard tabs). The synced view is kept in memory only, so it resets to each card's configured default on a full page reload. |
